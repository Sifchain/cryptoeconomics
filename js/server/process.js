...
exports.getProcessedLMData = snapshotLM => {
  ...
  const snapshotTimeseriesLength =
    minerSnapshotData[Object.keys(minerSnapshotData)[0]].rowan.length;
...
exports.getProcessedVSData = snapshotVS => {
  ...
  const snapshotTimeseriesLength =
    validatorSnapshotData[Object.keys(validatorSnapshotData)[0]].commission
      .length;
...
function processUserEventsByTimestamp (
  ...
  for (let i = 0; i < NUMBER_OF_INTERVALS_TO_RUN; i++) {
    ...
    let nextGlobalState;
    // Snapshot timeseries (generated by Vanir) overshoots by 1 extra interval
    const isSimulatedFutureInterval = i >= snapshotTimeseriesLength - 1;
