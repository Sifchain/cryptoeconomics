...
function remapVSAddresses(vaLAddresses, timeInterval) {
  ...
  mapped = _.map(vaLAddresses, (valAddressData, valStakeAddress) => {
    ...
    valDelegateEvents = _.map(delegates, (delegateAddress) => {
      const delegateTimeIntervals = valAddressData[delegateAddress].rowan
      return delegateTimeIntervals.map((amount, index) => {
        return {
          timestamp: (index + 1) * timeInterval,
          commision: commisionTimeIntervals[index],
          amount, delegateAddress, valSifAddress, valStakeAddress
        }
      }).filter(e => e.amount !== 0)
