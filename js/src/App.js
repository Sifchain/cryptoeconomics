...
class App extends React.Component {
  ...
  updateAddress(address) {
    window.history.pushState(undefined, '', `#${address}&type=${this.state.type}`)
    if (address !== 'all' && address !== 'none') {
      fetchUserTimeSeriesData(address, this.state.type).then(userTimeSeriesData => this.setState({ userTimeSeriesData }));
      fetchUserData(address, this.state.type).then(userData => this.setState({ userData }));
    }
    this.setState({
      address,
      userData: undefined,
  ...
  render() {
    ...
    const users = this.state.type === 'lm' ? this.state.usersLM : this.state.usersVS
      ...
      <div className="App" >
        <header className="App-header">
          <div className="radios">
            <label>
              <input type="radio" value="lm"
                onChange={this.updateType} checked={this.state.type === 'lm'} />
              Liquidity Pooling Rewards
            </label>
            <label>
              <input type="radio" value="vs"
                onChange={this.updateType} checked={this.state.type === 'vs'} />
          ...
          Address to show: <select value={this.state.address} onChange={this.updateAddressEvent}>
            ...
            {this.state.type === 'lm' && <option key={'all'} value={'all'}>All</option>}
        ...
        <div className='content'>
          ...
          {this.state.address !== 'all' && this.state.address !== 'none' && timeSeriesData && <input
            id="timestamp"
            ...
            step="1" />}
          {this.state.address !== 'all' && this.state.address !== 'none' &&
            <JSONPretty id="json-pretty" data={userTimestampJSON}></JSONPretty>}
        </div>
      </div >
    );
  }
